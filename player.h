...
{
    write_line("win state");
    string win_timer = "DoneTime";
    int time = timer_ticks(win_timer) / 1000;
    sprite player_sprite = this->player->get_player_sprite();
    start_timer(win_timer);
    if (!run_once)
    {
        sprite_set_dx(player_sprite, 0);
        sprite_set_dy(player_sprite, 20);
        sprite_start_animation(player->get_player_sprite(), "Dance");
        run_once = true;
    }
    ...
    {
        stop_timer(win_timer);
    }
}

void WinState::get_input()
{
}

// GameOver State Class
class GameOverState : public PlayerState
{
private:
    bool run_once = true;
...
{
    write_line("game over state");

    string game_over_timer = "Done";
    int time = timer_ticks(game_over_timer) / 1000;
    sprite player_sprite = this->player->get_player_sprite();
    start_timer(game_over_timer);
    if (!run_once)
    {
        sprite_set_dx(player_sprite, 0);
        sprite_set_dy(player_sprite, 10);
        sprite_start_animation(player->get_player_sprite(), "Dance");
        run_once = true;
    }
    ...
    {
        stop_timer(game_over_timer);
    }
}

void GameOverState::get_input() {}
